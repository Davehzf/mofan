...
public class ApiUserController extends ApiBaseAction {
	...
	public Object smscode(@LoginUser UserVo loginUser) {
		...
		System.out.println("jsonParams is:" + jsonParams);
		String phone = jsonParams.getString("phone");
		//String phone = "13723858886";
		// 一分钟之内不能重复发送短信
		...
		// 获取云存储配置信息
		/*
		 * SmsConfig config = sysConfigService.getConfigObject(Constant.SMS_CONFIG_KEY,
		 * SmsConfig.class); if (StringUtils.isNullOrEmpty(config)) { throw new
		 * RRException("请先配置短信平台信息"); } if (StringUtils.isNullOrEmpty(config.getName()))
		 * { throw new RRException("请先配置短信平台用户名"); } if
		 * (StringUtils.isNullOrEmpty(config.getPwd())) { throw new
		 * RRException("请先配置短信平台密钥"); } if (StringUtils.isNullOrEmpty(config.getSign()))
		...
		try {
			 ...
			 * "yyyy-MM-dd HH:mm:ss"), "");
			 */
			sendSmsResponse = SmsUtil.sendSms(phone, sms_code);
